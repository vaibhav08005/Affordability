...
import { mapNatWestRawInput } from "../dist/mappers/natwest/raw-to-lender-ready.js";

const inputDir = process.argv[2] ?? "samples/raw-halifax-cases";
const outputDir = process.argv[3] ?? "samples/natwest-mapped-cases";
...
for (const file of files) {
  ...
  const outputName = `${basename(file).replace(/^halifax-raw-/, "natwest-raw-").replace(/\.(yaml|yml|json)$/i, "")}.json`;
...
console.log(`Mapped ${files.length} NatWest raw case(s).`);
