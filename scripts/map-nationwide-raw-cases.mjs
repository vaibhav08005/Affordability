...
import { mapNationwideRawInput } from "../dist/mappers/nationwide/raw-to-lender-ready.js";

const inputDir = process.argv[2] ?? "samples/raw-halifax-cases";
const outputDir = process.argv[3] ?? "samples/nationwide-mapped-cases";
...
for (const file of files) {
  ...
  const result = mapNationwideRawInput(rawInput);
  const validated = lenderReadyInputSchema.parse(result.input);
  const outputName = `${basename(file).replace(/^halifax-raw-/, "nationwide-raw-").replace(/\.(yaml|yml|json)$/i, "")}.json`;
  const outputPath = join(outputDir, outputName);
  const payload = JSON.stringify(validated, null, 2);
  await writeFile(outputPath, `${payload}\n`, "utf8");

  console.log(`${file} -> ${outputPath}`);
  console.log("=== Nationwide final mapped output ===");
  console.log(payload);
...
console.log(`Mapped ${files.length} Nationwide raw case(s).`);
