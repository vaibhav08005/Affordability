...
import { mapHalifaxRawInput } from "../dist/mappers/halifax/raw-to-lender-ready.js";

const inputDir = process.argv[2] ?? "samples/raw-halifax-cases";
const outputDir = process.argv[3] ?? "samples/halifax-mapped-cases";
...
for (const file of files) {
  ...
  const outputName = `${basename(file).replace(/\.(yaml|yml|json)$/i, "")}.json`;
...
console.log(`Mapped ${files.length} Halifax raw case(s).`);
