...
import { mapVirginMoneyRawInput } from "../dist/mappers/virgin-money/raw-to-lender-ready.js";

const inputDir = process.argv[2] ?? "samples/raw-halifax-cases";
const outputDir = process.argv[3] ?? "samples/virgin-money-mapped-cases";
...
for (const file of files) {
  ...
  const result = mapVirginMoneyRawInput(rawInput);
  const validated = lenderReadyInputSchema.parse(result.input);
  const outputName = `${basename(file).replace(/^halifax-raw-/, "virgin-money-raw-").replace(/\.(yaml|yml|json)$/i, "")}.json`;
  const outputPath = join(outputDir, outputName);
  const payload = JSON.stringify(validated, null, 2);
  await writeFile(outputPath, `${payload}\n`, "utf8");

  console.log(`${file} -> ${outputPath}`);
  console.log("=== Virgin Money final mapped output ===");
  console.log(payload);
...
console.log(`Mapped ${files.length} Virgin Money raw case(s).`);
