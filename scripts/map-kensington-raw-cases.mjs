...
import { mapKensingtonRawInput } from "../dist/mappers/kensington/raw-to-lender-ready.js";

const inputDir = process.argv[2] ?? "samples/raw-halifax-cases";
const outputDir = process.argv[3] ?? "samples/kensington-mapped-cases";
...
for (const file of files) {
  ...
  const outputName = `${basename(file).replace(/^halifax-raw-/, "kensington-raw-").replace(/\.(yaml|yml|json)$/i, "")}.json`;
...
console.log(`Mapped ${files.length} Kensington raw case(s).`);
