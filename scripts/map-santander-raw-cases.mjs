...
import { mapSantanderRawInput } from "../dist/mappers/santander/raw-to-lender-ready.js";

const inputDir = process.argv[2] ?? "samples/raw-halifax-cases";
const outputDir = process.argv[3] ?? "samples/santander-mapped-cases";
...
for (const file of files) {
  ...
  const outputName = `${basename(file).replace(/^halifax-raw-/, "santander-raw-").replace(/\.(yaml|yml|json)$/i, "")}.json`;
...
console.log(`Mapped ${files.length} Santander raw case(s).`);
