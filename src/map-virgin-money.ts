...
async function main() {
  ...
  if (!inputPath) {
    throw new Error("Usage: tsx src/map-virgin-money.ts <raw-input.yaml|json> [output-lender-ready.json]");
  }
  ...
  if (outputPath) {
    ...
    console.log(`=== Virgin Money mapped output written to ${outputPath} ===`);
  }

  if (result.issues.length > 0) {
    console.log("=== Virgin Money mapping issues ===");
