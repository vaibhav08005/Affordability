...
async function main() {
  ...
  if (!inputPath) {
    throw new Error("Usage: tsx src/map-nationwide.ts <raw-input.yaml|json> [output-lender-ready.json]");
  }
  ...
  if (outputPath) {
    ...
    console.log(`=== Nationwide mapped output written to ${outputPath} ===`);
  }

  if (result.issues.length > 0) {
    console.log("=== Nationwide mapping issues ===");
