...
export interface BarclaysRawMappingResult {
...
export function mapBarclaysRawInput(raw: RawRecord): BarclaysRawMappingResult {
  ...
  const applicants = buildApplicants(raw, numberOfApplicants, issues, loanAmount);
  const postcode = propertyPostcode(raw);
  const otherProperties = mapBarclaysOtherProperties(raw);
  const otherMortgageCommitments = mapResidentialMortgageCommitments(raw);
  const sharedOwnershipScheme = mapSharedOwnershipFlag(raw) ? mapSharedOwnershipScheme(raw) : undefined;
  ...
  const input: LenderReadyInput = {
    ...
    case: {
      ...
      sharedOwnershipOrEquity: isBarclaysEquityLoanCase(raw),
      ...
      equityLoanBalance: isBarclaysEquityLoanCase(raw) ? rawNumber(raw.var_htb_loan_amount ?? raw.var_shared_equity_loan_amount ?? raw.var_value_of_share, 0) : undefined,
      equityLoanInterestRatePercent: isBarclaysEquityLoanCase(raw) ? 0 : undefined,
      ...
      monthlyRepaymentPlanPremium: undefined
    },
    property: {
      isInScotland: isScottishPostcode(postcode),
      tenure: mapTenure(raw, isScottishPostcode(postcode)),
      epcRating: "unknown"
    },
    loan: {
      propertyValue,
    ...
    applicants,
    outgoings: {
      monthlyLoanRepayments: mapMonthlyFinancialCommitments(raw),
  ...
  if (repaymentType === "part_and_part") {
    issues.push({
      field: "var_repayment_type",
      message: "Barclays adapter currently maps part-and-part into the calculator repayment path without splitting mortgage parts."
    });
  }
...
function mapCustomerType(raw: RawRecord): "first_time_buyer" | "home_mover" {
  return normalized(raw.var_journey) === "ftb" ? "first_time_buyer" : "home_mover";
}
...
function mapPropertyValue(raw: RawRecord, mortgagePurpose: MortgagePurpose): number {
  const ownershipType = normalized(raw.var_ownership_type);
...
function mapLoanAmount(raw: RawRecord, mortgagePurpose: MortgagePurpose, propertyValue: number, deposit: number): number {
  ...
  if (mortgagePurpose === "purchase") {
    const schemeLoan = ownershipType.includes("shared") || ownershipType.includes("htb")
...
function buildApplicants(raw: RawRecord, numberOfApplicants: 1 | 2, issues: MappingIssue[], loanAmount: number): Applicant[] {
  const applicants: Applicant[] = [];
  for (let index = 1; index <= numberOfApplicants; index += 1) {
    applicants.push(buildApplicant(raw, index as 1 | 2, issues, loanAmount));
  }
  return applicants;
}
...
function mapEmployment(raw: RawRecord, index: 1 | 2, loanAmount: number): Applicant["employment"] {
  const prefix = `var_appl${index}`;
  ...
  const type = inferEmploymentType(raw, prefix, employmentType, businessType, ownershipPercentage, isContractor);
  ...
  if (type === "self_employed") {
    ...
    const profits = mapSelfEmployedProfits(raw, prefix, employment.businessType, loanAmount);
...
function inferEmploymentType(
  ...
  if (["retired", "pension", "home_maker", "homemaker", "not_working"].some((token) => employmentType.includes(token))) return "pension";
  if (isContractor) return "employed";
  if (
    businessType ||
    ownershipPercentage >= 20 ||
    employmentType.includes("self") ||
    (employmentType === "" && (rawNumber(raw[`${prefix}_land_curr_profit`], 0) > 0 || rawNumber(raw[`${prefix}_land_prev_profit`], 0) > 0))
  ) {
...
function mapSelfEmployedProfits(raw: RawRecord, prefix: string, businessType: SelfEmploymentType, loanAmount: number): { current: number; previous: number } {
  ...
  const landCurrent = rawNumber(raw[`${prefix}_land_curr_profit`], 0);
  const landPrevious = rawNumber(raw[`${prefix}_land_prev_profit`], 0);
  if (landCurrent > 0 || landPrevious > 0) return { current: landCurrent, previous: landPrevious };
  if (businessType === "sole_trader") {
    return {
      current: rawNumber(raw[`${prefix}_st_curr_yr_profit`] ?? raw[`${prefix}_business_curr_yr_net_profit`], 0),
      previous: rawNumber(raw[`${prefix}_st_prev_yr_profit`] ?? raw[`${prefix}_business_prev_yr_net_profit`], 0)
    };
  }
  ...
  return {
    current: salary + Math.max(rawNumber(raw[`${prefix}_business_curr_yr_share_profit`], 0), rawNumber(raw[`${prefix}_business_curr_yr_dividends`], 0)),
    previous: salary + Math.max(rawNumber(raw[`${prefix}_business_prev_yr_share_profit`], 0), rawNumber(raw[`${prefix}_business_prev_yr_dividends`], 0))
  };
}
...
function mapMonthlyFinancialCommitments(raw: RawRecord): number {
...
function mapBarclaysOtherProperties(raw: RawRecord): LenderReadyInput["otherProperties"] {
...
function isBarclaysEquityLoanCase(raw: RawRecord): boolean {
  const ownershipType = normalized(raw.var_ownership_type || raw.var_ownership_type_ftb);
  return ownershipType.includes("htb") || ownershipType.includes("help_to_buy") || ownershipType.includes("shared_equity") || ownershipType.includes("forces");
}
...
function mapBusinessType(value: unknown): SelfEmploymentType | undefined {
  ...
  if (businessType.includes("limited")) return "limited_company";
  if (businessType.includes("partnership")) return "partnership";
  if (businessType.includes("llp")) return "llp";
  if (businessType.includes("sole")) return "sole_trader";
  return undefined;
}

function annualizeBarclays(amount: unknown, frequency: unknown): number {
