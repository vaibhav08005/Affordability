...
function mapSelfEmployedProfits(raw: RawRecord, prefix: string, businessType: SelfEmploymentType): { current: number; previous: number } {
  ...
  if (businessType === "limited_company") {
    ...
    return {
      current: Math.min(currentDividends || currentProfit, currentProfit || currentDividends),
      previous: Math.min(previousDividends || previousProfit, previousProfit || previousDividends)
    };
  }
