...
export interface HalifaxRawMappingResult {
...
export function mapHalifaxRawInput(raw: RawRecord): HalifaxRawMappingResult {
...
function mapMortgagePurpose(raw: RawRecord): MortgagePurpose {
  ...
  if (additionalBorrowing > 0 || hasValue(raw.var_add_borrow_details)) {
    return "remortgage_capital_raising";
  }

  return "remortgage_no_additional_borrowing";
}
...
function mapPropertyValue(raw: RawRecord, mortgagePurpose: MortgagePurpose): number {
  ...
  if (ownershipType.includes("shared_ownership") && valueOfShare > 0) {
    return valueOfShare;
  }

  if (mortgagePurpose === "purchase") {
    return rawNumber(raw.var_property_value, 0);
  }
...
function mapLoanAmount(raw: RawRecord, mortgagePurpose: MortgagePurpose, propertyValue: number, deposit: number): number {
  ...
  if (mortgagePurpose === "further_advance") {
    return nonNegative(additionalBorrowing);
  }
...
function mapSharedEquityCustomerStakePercent(raw: RawRecord, propertyValue: number): number {
  ...
  if (propertyValue > 0 && explicitShareValue > 0) {
    return roundPercent((explicitShareValue / propertyValue) * 100);
  }

  const equityLoan = rawNumber(raw.var_htb_loan_amount ?? raw.var_shared_equity_loan_amount, 0);
  if (propertyValue > 0 && equityLoan > 0) {
    return roundPercent(((propertyValue - equityLoan) / propertyValue) * 100);
  }
...
function mapMonthlyRepaymentPlanPremium(raw: RawRecord, applicants: Applicant[]): number | undefined {
  const repaymentType = mapRepaymentType(raw.var_new_repayment_type ?? raw.var_repayment_type);
  if (repaymentType === "capital_and_interest") return undefined;
  if (normalized(raw.var_repay_vehicle_type).includes("main_residence")) return 0;
...
function buildApplicant(raw: RawRecord, index: 1 | 2, issues: MappingIssue[]): Applicant {
  ...
  const age = dateOfBirth ? ageFromEpoch(dateOfBirth) : 35;
  if (!dateOfBirth) {
    issues.push({
...
function mapEmployment(raw: RawRecord, index: 1 | 2): Applicant["employment"] {
  ...
  const type: EmploymentType = inferEmploymentType(employmentType, businessType, ownershipPercentage, tenPercentShare);
...
function mapAnnualBonus(raw: RawRecord, prefix: string): number {
  ...
  return Math.min(recent, (recent + previous) / 2);
}
...
function mapSelfEmployedProfits(raw: RawRecord, prefix: string, businessType: SelfEmploymentType): { current: number; previous: number } {
  ...
  if (businessType === "llp") {
    const currentShare = rawNumber(raw[`${prefix}_business_curr_yr_share_profit`], 0);
    const previousShare = rawNumber(raw[`${prefix}_business_prev_yr_share_profit`], 0);
  ...
  return {
    current: salary + rawNumber(raw[`${prefix}_business_curr_yr_share_profit`] ?? raw[`${prefix}_business_curr_yr_net_profit`], 0),
    previous: salary + rawNumber(raw[`${prefix}_business_prev_yr_share_profit`] ?? raw[`${prefix}_business_prev_yr_net_profit`], 0)
  };
}
...
function mapDependants(raw: RawRecord, numberOfApplicants: 1 | 2): LenderReadyInput["household"]["dependants"] {
  const dependants = [
    ...rawArray(raw.var_appl1_dependents_details),
    ...rawArray(raw.var_appl2_dependents_details)
  ].map((item) => {
...
function mapMonthlyLoanRepayments(raw: RawRecord): number {
  return (
    sumCreditCommitments(raw, ["loans", "loan", "student_loans", "student_loan", "secured_loans", "hire_purchase", "lease", "buy_now_pay_later"]) +
...
function isBuyToLetProperty(property: RawRecord): boolean {
  const rentalFlag = yes(property.is_rental_property);
  const occupancyStatus = normalized(property.occupancy_status);
  return rentalFlag || ["let", "to_be_let", "rental_property_already_let"].includes(occupancyStatus);
}
...
function propertyPostcode(raw: RawRecord): string {
  ...
  if (propertyAddress && typeof propertyAddress === "object") {
    return rawString(propertyAddress.postcode);
  }
  return "";
}
