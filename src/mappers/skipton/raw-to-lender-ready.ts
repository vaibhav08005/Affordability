...
export interface SkiptonRawMappingResult {
...
export function mapSkiptonRawInput(raw: RawRecord): SkiptonRawMappingResult {
  ...
  if (!postcode) {
    issues.push({
      ...
      message: "Property address postcode was not present; Skipton region defaults to South East through the adapter."
    });
  }
  ...
  if (sharedOwnershipScheme === "shared_equity") {
    issues.push({
      ...
      message: "Skipton workbook includes Help to Buy/shared equity loan balance, but the current Skipton adapter only submits the shared-ownership rent expenditure field."
    });
  }
...
function mapEmployment(raw: RawRecord, index: 1 | 2, issues: MappingIssue[]): Applicant["employment"] {
  ...
  const employedStartDate = raw[`${prefix}_employed_permanent_temporary_start_date`];
  const businessStartDate = raw[`${prefix}_business_start_date`];
  const employedMonths = monthsSinceEpoch(employedStartDate);
  const selfEmployedMonths = monthsSinceEpoch(businessStartDate);
  const hasSufficientEmploymentHistory = type !== "employed" || employedMonths == null || employedMonths >= 6;
  ...
  if (!hasSufficientSelfEmploymentHistory) {
    issues.push({
      ...
      message: "Skipton workbook says self-employed income should be set to 0 where trading is under 24 months."
    });
  }
...
function mapSelfEmployedProfits(raw: RawRecord, prefix: string, businessType: SelfEmploymentType): { current: number; previous: number } {
  if (businessType === "limited_company") {
    ...
    return {
      current: twoYearLowerOrAverage(currentDividends, previousDividends),
      previous: previousDividends
    };
  }

  const current = rawNumber(
    raw[`${prefix}_st_curr_yr_profit`] ??
      raw[`${prefix}_business_curr_yr_share_profit`] ??
      raw[`${prefix}_business_curr_yr_net_profit`],
    0
  );
  const previous = rawNumber(
