...
async function main() {
  ...
  if (!inputPath) {
    throw new Error("Usage: tsx src/map-hsbc.ts <raw-input.yaml|json> [output-lender-ready.json]");
  }
  ...
  if (outputPath) {
    ...
    console.log(`=== HSBC mapped output written to ${outputPath} ===`);
  }

  if (result.issues.length > 0) {
    console.log("=== HSBC mapping issues ===");
